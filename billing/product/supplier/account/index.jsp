...
try {
    Vector tots = bill.getSupplierAccountTotals();
    if (tots != null && tots.size() >= 3) {
        totalSuppliers = Long.parseLong(tots.get(0).toString());
        totalDue       = Double.parseDouble(tots.get(1).toString());
        totalAdvance   = Double.parseDouble(tots.get(2).toString());
    }
    dueList = bill.getSuppliersDueList();
} catch(Exception _e) {}
%>
<!DOCTYPE html>
<html lang="en">
...
<%
    request.setAttribute("pageTitle",    "Suppliers Balance");
    request.setAttribute("pageSubtitle", "Credit Management — Supplier Account");
  ...
  <div class="row g-3 mb-4">
    <div class="col-4">
      <div class="mst-card p-3 text-center">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;opacity:.7;">Due Suppliers</div>
        <div style="font-size:28px;font-weight:900;"><%=totalSuppliers%></div>
      </div>
    </div>
    <div class="col-4">
  ...
  <div class="mst-card">
    <div class="p-4">
            <h5 class="mb-4" style="font-weight:700;">
                <i class="fa-solid fa-magnifying-glass me-2"></i>Search Supplier
            </h5>

            <div style="position:relative;">
                <input type="text" id="supplierSearch" class="form-control fg-inp"
                       placeholder="Type name or phone number..." autocomplete="off"
                       style="font-size:15px;height:42px;padding-right:40px;">
                <i class="fa-solid fa-truck-field" style="position:absolute;right:13px;top:50%;transform:translateY(-50%);opacity:.4;"></i>
                <ul id="supplierDropdown" style="
            ...
            <div id="selectedSupplierBox" style="display:none;margin-top:18px;
...
searchInput.addEventListener('input', function() {
    ...
    searchTimer = setTimeout(() => {
        ...
        fetch(contextPath + '/product/supplier/account/supplierAutocomplete.jsp?' + params)
            .then(r => r.json())
...
function selectSupplier(s) {
    ...
    document.getElementById('selPhone').textContent = s.phone && s.phone !== '-' ? s.phone : '';
    document.getElementById('selectedSupplierBox').style.display = 'block';
}

function goToView() {
    if (!selectedSupplierId) return;
    window.location.href = contextPath + '/product/supplier/account/view.jsp?supplierId=' + selectedSupplierId;
}
...
searchInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && selectedSupplierId) goToView();
});
</script>
...
<div id="dueModal" style="display:none;position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.45);align-items:center;justify-content:center;padding:16px;">
  <div style="background:#fff;border-radius:12px;width:100%;max-width:560px;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 8px 40px rgba(0,0,0,.22);">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #e5e7eb;">
      <div>
        <div style="font-weight:800;font-size:15px;"><i class="fa-solid fa-triangle-exclamation me-2" style="color:#dc2626;"></i>Suppliers with Due</div>
        <div style="font-size:12px;opacity:.6;margin-top:2px;"><%=dueList.size()%> supplier<%=dueList.size() != 1 ? "s" : ""%> &nbsp;|&nbsp; Total: &#8377;<%=String.format("%,.2f", totalDue)%></div>
      </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <thead>
          <tr style="background:#f8fafc;position:sticky;top:0;">
            ...
            <th style="padding:10px 16px;text-align:left;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.5px;">Supplier</th>
        ...
        <tbody>
          <% if (dueList.isEmpty()) { %>
          <tr><td colspan="4" style="padding:24px;text-align:center;color:#888;">No suppliers with outstanding due.</td></tr>
          <%
          } else {
            for (int i = 0; i < dueList.size(); i++) {
              Vector dr = (Vector) dueList.get(i);
              String dSupId  = dr.get(0).toString();
          ...
          %>
          <tr onclick="window.location.href='<%=request.getContextPath()%>/product/supplier/account/view.jsp?supplierId=<%=dSupId%>'"
