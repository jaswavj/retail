...
<body>
    ...
    <div class="container my-5">
        ...
        <table id="cancelBillTable" class="table table-hover" style="min-width: 800px;">
            ...
            <tbody>
                <%
                Vector list = bill.getCancelBill(fromDate, toDate);
                if (list != null && !list.isEmpty()) {
                    for (int i = 0; i < list.size(); i++) {
                        Vector row = (Vector) list.get(i);
                        int billId = Integer.parseInt(row.elementAt(7).toString());
                %>
                <tr>
                    <td><%=i+1%></td>
...
function exportTableToExcel() {
    const table = document.getElementById('cancelBillTable');
    const filename = 'Cancelled_Bills_Report.xls';
