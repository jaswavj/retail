...
try {
    ...
    if (custIdStr == null || entryType == null) {
    ...
    if ("OLD_DUE".equals(type)) {
        ...
        double newBalance = bill.saveCustomerOldDue(customerId, dueAmount, userId, noteText);
    ...
    if ("ADVANCE".equals(type)) {
        ...
        double newAdvance = bill.saveCustomerAdvance(customerId, cashPaid, bankPaid, payMode, payType, userId, noteText);
        out.print("{\"success\":true,\"entryType\":\"ADVANCE\",\"newAdvance\":" + newAdvance + "}");
        return;
    }

    if ("COLLECTION".equals(type)) {
        Vector acc = bill.getCustomerAccount(customerId);
        double accBal = 0, accAdv = 0;
        if (acc != null && acc.size() >= 4) {
            try { accAdv = Double.parseDouble(acc.get(2).toString()); } catch (Exception ignore) {}
            try { accBal = Double.parseDouble(acc.get(3).toString()); } catch (Exception ignore) {}
        }
        if (accBal <= 0) {
            out.print("{\"success\":false,\"message\":\"No account balance due to collect.\"}");
            return;
        }
        double netDue = accBal - accAdv;
        if (netDue <= 0) {
            out.print("{\"success\":false,\"message\":\"Nothing to collect. Account balance is covered by advance.\"}");
            return;
        }
        double cashPaid = (cashStr != null && !cashStr.trim().isEmpty()) ? Double.parseDouble(cashStr.trim()) : 0;
        double bankPaid = (bankStr != null && !bankStr.trim().isEmpty()) ? Double.parseDouble(bankStr.trim()) : 0;
    ...
    if (msg.toLowerCase().contains("txn_type") || msg.toLowerCase().contains("unknown column")) {
        msg = "Database update required. Please run database/customer_account_txn_type.sql";
    }
