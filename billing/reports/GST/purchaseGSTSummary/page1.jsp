...
Vector list = prod.getPurchaseGSTSummary(from,end);
...
<head>
  ...
  <title>Purchase GST Summary Report</title>
  ...
  <div class="container my-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div class="text-center flex-grow-1">
        <h1 class="report-title">Purchase GST Summary Report</h1>
        <p class="text-muted mb-0">From <strong><%=from%></strong> To <strong><%=end%></strong></p>
      </div>
      <div class="no-print">
        <a href="<%=contextPath%>/reports/GST/purchaseGSTSummary/page.jsp" class="btn btn-secondary me-2">⬅ Back</a>
    ...
    <div class="table-responsive shadow-sm rounded">
      <table id="purchaseGstSummaryTable" class="table table-hover align-middle">
        <thead>
          <tr style="background: linear-gradient(135deg, #f7fafc 0%, #edf2f7 100%); border-bottom: 2px solid #e2e8f0;">
            ...
            <th style="padding: 0.4rem; font-size: 0.85rem; font-weight: 600; color: #4a5568; text-align: right;">Taxable Value (₹)</th>
...
function exportTableToExcel() {
    const table = document.getElementById('purchaseGstSummaryTable');
    const filename = 'Purchase_GST_Summary_Report.xls';
...
function exportToJSON() {
    // Calculate taxable and nil-rated values for purchase (ITC eligible)
    let taxableValue = 0;
    let totalCGST = 0;
    let totalSGST = 0;
    let nilRatedValue = 0;
    ...
    const gstr3bData = {
        ...
        "inward_sup": {
            "isup_details": [
                {
                    "ty": "GST",
                    "intra": parseFloat(taxableValue.toFixed(3)),
                    "inter": 0
                },
                {
                    "ty": "NONGST",
                    "intra": parseFloat(nilRatedValue.toFixed(3)),
                    "inter": 0
                }
            ]
        }
    ...
    const downloadLink = document.createElement('a');
    downloadLink.href = url;
    downloadLink.download = 'GSTR3B_Purchase_ITC_<%=from%>_to_<%=end%>.json';
    document.body.appendChild(downloadLink);
    downloadLink.click();
    document.body.removeChild(downloadLink);
    URL.revokeObjectURL(url);
